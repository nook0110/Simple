...
TEST(PromotionMoves, AllPieces) {
    ...
    const Move knight_promo = Move::Make<MoveType::kPromotion>(48, 56, Piece::kKnight);
    ASSERT_EQ(knight_promo.PromotionPiece(), Piece::kKnight);
    
    const Move rook_promo = Move::Make<MoveType::kPromotion>(48, 56, Piece::kRook);
    ASSERT_EQ(rook_promo.PromotionPiece(), Piece::kRook);
    
    const Move bishop_promo = Move::Make<MoveType::kPromotion>(48, 56, Piece::kBishop);
...
TEST(HashFunction, Consistency) {
    ...
    Move::Hash hasher;
    ASSERT_EQ(hasher(move1), hasher(move2));
    ASSERT_NE(hasher(move1), hasher(move3));
}
